    ...
    if (this.props.isLoaded && this.props.genreLoaded) {
      return (
        <div className="Poster">
          {this.props.movie.length === 0 ? <p style={{fontFamily: 'Inter UI', fontSize: '20px',textAlign: 'left', margin: 50, marginBottom: '75%'}}>Sorry, your search did not match any results.</p> : this.props.movie.map((item) => {
            ...
            return (
              <section className="posterBlock" key={item.id}>
                ...
                <div className="informationBlock">
                  ...
                  <h5>{item.title}</h5>
                  <div className="Vote" style={styles}>{item.vote_average.toFixed(1)}</div>
                  </div>
